...
const product = JSON.parse(await readFile(new URL('../brand/chimera/product.json', import.meta.url), 'utf8'));
const versionName = `${product.upstreamAppVersion}-chimera.${product.chimeraRevision}`;
assert.equal(config.name, 'Chimera'); assert.equal(config.slug, 'chimera');
assert.equal(config.version, versionName); assert.equal(config.runtimeVersion, versionName);
assert.equal(config.android.versionCode, product.androidVersionCode);
