...
function Protect-PrivatePath([string] $Path) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Unable to apply restrictive ACLs to private signing material.' }
}
...
$keytool = Find-CommandPath 'keytool.exe' @('D:\Desktop\Hack\tools\jdk21\bin\keytool.exe')
$openssl = Find-CommandPath 'openssl.exe' @('D:\Scoop\apps\git\current\mingw64\bin\openssl.exe')
...
if ($OfflineRecoveryRotation) { throw 'Offline recovery/rotation is intentionally manual and is not supported by this bootstrap command.' }
...
try {
    ...
    & $keytool -genkeypair -noprompt -storetype JKS -keystore $keystore -storepass $storePassword -keypass $keyPassword -alias chimera-release -keyalg RSA -keysize 4096 -sigalg SHA256withRSA -validity 10000 -dname 'CN=Chimera Release, OU=Chimera, O=Chimera, L=Beijing, C=CN' 2>$null | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'Android JKS signing key generation failed.' }
    $certificate = & $keytool -list -v -keystore $keystore -storepass $storePassword -alias chimera-release 2>&1
    ...
    $bundle = $bundlePath
    & $openssl enc -aes-256-cbc -pbkdf2 -iter 600000 -salt -in $archive -out $bundle -pass "pass:$storePassword" 2>$null
