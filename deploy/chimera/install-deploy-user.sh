...
[[ "$#" -eq 4 ]] || { printf 'usage: %s <server-key> <android-key> <web-key> <status-key>\n' "$0" >&2; exit 2; }
server_key_file="$1"
android_key_file="$2"
web_key_file="$3"
status_key_file="$4"
...
status_key="$(tr -d '\r\n' < "$status_key_file")"
[[ "$status_key" =~ ^ssh-ed25519\ [A-Za-z0-9+/=]+([[:space:]][A-Za-z0-9._@-]+)?$ ]] || exit 1
...
install -d -m 0755 /opt/chimera /opt/chimera/state /opt/chimera/downloads /opt/chimera/downloads/releases /opt/chimera/web /opt/chimera/web/releases /opt/chimera/config /opt/chimera/proxy-config
